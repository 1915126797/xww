...
function check(){
    xiha.postData("api.php?do=check","r="+Math.random(1),function(d){
       if(d.code==200||d.code==201||d.code==400){
			...
			$('#submit').hide();
			$("#load").show();		
			$('#load').html('<div class="alert alert-success">检测到登录记录！</div><button type="button" id="sign" class="btn btn-primary btn-block">签到</button><br/><button type="button" id="daka" class="btn btn-primary btn-block">打卡</button><br/><a id="relogin">返回重新登录</a>');
...
function login(uin,pwd){
	...
	xiha.postData(loginurl,"uin="+uin+"&pwd="+md5(pwd)+"&r="+Math.random(1), function(d) {
		if(d.code ==200){
			...
			$('#login').hide();		
			$('#submit').hide();
			$('#load').html('<div class="alert alert-success">登录成功！'+decodeURIComponent(d.profile.nickname)+'</div><button type="button" id="sign" class="btn btn-primary btn-block">签到</button><br/><button type="button" id="daka" class="btn btn-primary btn-block">打卡</button><br/><a href="./">返回重新登录</a>');
			$('#sign').click(function(){sign();});
			$('#daka').click(function(){daka();});
		}else if(d.code ==502){
			$('#load').html('密码错误！');
